...
class AsyncQueue
{
    ...
    public static function submitQuery(AsyncQuery $query, ?callable $callbackFunction = null, ?array $valuesToPass = null)
    {
        self::$callbacks[spl_object_hash($query)] = $callbackFunction;
        self::$values[spl_object_hash($query)] = $valuesToPass;
        $query->host = Settings::$host;
        $query->user = Settings::$username;
        $query->password = Settings::$password;
        GreekNetwork::getInstance()->getServer()->getAsyncPool()->submitTask($query);
    }
    ...
    public static function activateCallback(AsyncQuery $query): void
    {
        ...
        $values = self::$values[spl_object_hash($query)] ?? null;
        if (is_callable($callable)) {
            $callable($query["rows"], $values);
        }
    }
}